...
export default function ProfileScreen({route,navigation}) {
 ...
 const isFocused = useIsFocused();
  ...
  const fetchReport = async ()=>{
    ...
    fetch("http://10.43.11.197:3000/createReport",{
    method: "POST",
  ...
  useFocusEffect(
    ...
    useEffect(() => {
      ...
      if(retreive){      
        Promise.all([
          fetch('http://10.43.11.197:3000/getMyAccountInfo',{
            method: "POST",
            headers: {'content-type': 'application/json'},
            body: JSON.stringify({
              "id":profile_id,
            })}),
          fetch('http://10.43.11.197:3000/getReviewUser',{
            method: "POST",
            headers: {'content-type': 'application/json'},
            body: JSON.stringify({
              "id":profile_id,
            })}),
          fetch('http://10.43.11.197:3000/getReportTypes'),
