...
export default function ParticipationDemandScreen({route,navigation}) {
 ...
 /*  const out =0
  ...
  const refuseFetch = async()=>{
    fetch("http://192.168.98.166:3000/refuseDemand",{
      method: "POST",
  ...
  const acceptFetch = async()=>{
    fetch("http://192.168.98.166:3000/acceptDemand",{
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  const signoutFetch = async ()=>{
    fetch("http://192.168.98.166:3000/signoutDemand",{
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  useEffect(() => {
    ...
    if(retreive){      
      Promise.all([
        ...
        fetch('http://192.168.98.166:3000/getInfoDemanderNotif',{
          method: "POST",
          headers: {'content-type': 'application/json'},
          body: JSON.stringify({
            "id":notif_id
